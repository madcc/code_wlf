...
u8 lcdSerial_displayfunction;
u8 lcdSerial_displaycontrol;
u8 lcdSerial_displaymode;

u8 lcdSerial_initialized;

u8 lcdSerial_xcursor;
u8 lcdSerial_ycursor;

u8 lcdSerial_numLines,lcdSerial_numCols;
...
void lcdSerialBegin(u8 cols, u8 lines)
{
	// program special characters
    u8 right[] = {0,4,2,1,2,4,0};
    u8 down[] = {0,0,0,0,0,0x11,0xA,4};
...
void lcdSerialClear()
{
  ...
  delay_us(2000);  // this lcdSerialCommand takes a long time!
}
...
void lcdSerialSetCursor(u8 col, u8 row)
{
...
void lcdSerialCreateChar(u8 location, u8 charmap[]) 
{
	int i;
...
void lcdSerialCommand(u8 value) {
  lcdSerialSend(value, false);
}

void lcdSerialWrite(u8 value) {
...
void lcdSerialSend(u8 value, bool mode)
{
	u8 modeBits;
	// set mode value
	if(mode)
 ...
 //serial assumes 4 bit mode
	...
	delay_us(200);//according to the datasheet,should check the busy signal,but we can not read ,so delay.
					//however ,without the delay ,it can work. RY
	//lcdSerialCheckBusy();
}
void lcdSerialRead(u8 value, bool mode)
{
	u8 modeBits;
	if(mode)
		modeBits=0x06;
...
//   value |= 0x08;
//   lcdSerialWriteSerial(value);
//  delay_us(4);    // enable pulse must be >450ns  //may be this can be delay_us(1);RY
//   //set enable to false
//   //value &= 0b11110111;
//   value &= 0xF7;
//   lcdSerialWriteSerial(value);
//  delay_us(1);   // lcdSerialCommands need > 37us to settle [citation needed]
//}
//
//void lcdSerialWriteSerial(u8 value) 
//{
//  
//  int i;
//  
...
//	  
//	  delay_us(4); //may be this can be delay_us(1);RY
